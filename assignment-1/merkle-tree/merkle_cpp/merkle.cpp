...
void HashedNode_4_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void HashedNode_4_run(uint ctx_index,Circom_CalcWit* ctx);
void Merkle_5_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Merkle_5_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 5331;}

uint get_number_of_components() {return 17;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 3965;}
...
ctx->componentMemory[coffset].templateName = "HashedNode";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
for (uint i = 0; i < 4; i++) {
std::string new_cmp_name = "hashedLeaf"+ctx->generate_position_array(aux_dimensions, 1, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
...
std::string new_cmp_name = "hashedNode"+ctx->generate_position_array(aux_dimensions, 1, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
HashedNode_4_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1773 ;
aux_cmp_num += 4;
}
}
...
Fr_lt(&expaux[0],&lvar[5],&circuitConstants[9]); // line circom 44
while(Fr_isTrue(&expaux[0])){
{
...
{
PFrElement aux_dest = &lvar[((1 * Fr_toInt(&lvar[5])) + 1)];
// load src
// end load src
...
PFrElement aux_dest = &lvar[5];
// load src
Fr_add(&expaux[0],&lvar[5],&circuitConstants[3]); // line circom 44
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[5],&circuitConstants[9]); // line circom 44
...
Fr_gt(&expaux[0],&lvar[6],&circuitConstants[3]); // line circom 56
while(Fr_isTrue(&expaux[0])){
Fr_lt(&expaux[0],&lvar[5],&circuitConstants[5]); // line circom 57
while(Fr_isTrue(&expaux[0])){
{
...
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
HashedNode_4_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &lvar[6];
// load src
Fr_div(&expaux[0],&lvar[6],&circuitConstants[5]); // line circom 68
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_gt(&expaux[0],&lvar[6],&circuitConstants[3]); // line circom 56
...
Merkle_5_create(1,0,ctx,"main",0);
Merkle_5_run(0,ctx);
}
